...
# Construct job name
job_name="${model}_${hidden_dim}_${numneg}_${comment}"

# Format time limit as a string
time_limit="${time_limit_hours}:00:00"

cmd="python ./train.py $model $sample_type $numneg $batch $epochs $item_freeze $user_freeze $comment $user_init $item_dynamic_unfreeze $hidden_dim $use_confidence $l2"
