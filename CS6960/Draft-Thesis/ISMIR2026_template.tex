...
While larger models are often assumed to provide superior representations, their effectiveness in limited, single-domain settings remains unclear.
We evaluate multiple CNN-based and Transformer-based embedding models across different capacity tiers using two proxy tasks: composer-based retrieval as a structured baseline and musical-character retrieval as a semantic task. Retrieval performance is assessed using standard ranking metrics, including NDCG, precision, recall, and F1.
The results show that increasing model capacity does not consistently improve retrieval performance. While higher-capacity models provide some benefit for structured tasks, these gains are limited and tend to saturate. For abstract semantic retrieval, performance differences across capacity levels are minimal, indicating weak or absent scaling effects.
These findings support a condition-dependent view of model scaling, where the effectiveness of increased capacity depends on task structure, label quality, and dataset characteristics. In small-scale archival settings, lightweight or mid-sized models can offer competitive performance with lower computational cost.
...
Performance is primarily assessed using NDCG@10 and Precision@10, with interpretation focused on
...
However, gains are not proportional to model size, indicating diminishing returns at higher capacity.
...
than the small model rather than showing a consistent scaling trend. Within the Transformer family,
the medium and large models perform at approximately the same level, indicating no meaningful
...
impose a coarser and less structurally constrained notion of similarity. This indicates that scaling alone is insufficient for improving abstract semantic retrieval in this setting.
...
guarantee monotonic performance improvement. On the Sanity Proxy, capacity helps to some extent, but
...
This can be explained by the limited diversity and scale of the dataset.
In such settings, the representational space required to separate items is relatively low-dimensional,
reducing the benefit of high-capacity models.
...
The results suggest that higher-capacity models provide measurable benefits only when the retrieval task is aligned with structured and well-defined attributes, such as composer identity.
...
The experimental results support the hypothesis that increasing embedding-model capacity does not necessarily improve retrieval performance in a small-scale, single-domain archive, reinforcing a condition-dependent view of model scaling.

\subsection{Implications for Backend Design in Classical Music Archives}

The results suggest that, for small-scale classical music archives, increasing embedding-model capacity is not the most effective strategy for improving retrieval performance.
Instead, lightweight or mid-sized models may provide comparable performance at lower computational cost.

These findings indicate that system design should prioritize task definition, label quality, and dataset curation over model scaling.
In practical archival systems such as iPalpiti, this supports the use of efficient models for real-time retrieval and deployment-constrained environments.
...
\section{Conclusion}
This study investigated the effect of embedding-model capacity on content-based music similarity retrieval in a small-scale classical music archive.
...
These findings support a condition-dependent view of model scaling, where the effectiveness of increased capacity depends on task structure, label quality, and dataset characteristics. In small, single-domain archives, factors such as dataset size and semantic definition play a more significant role than model capacity alone.

From a practical perspective, the results suggest that lightweight or mid-sized models are often sufficient for deployment in classical music archival systems, offering competitive performance with lower computational cost. Overall, this work highlights the importance of aligning model complexity with task requirements and dataset constraints, rather than assuming that larger models will universally yield better retrieval performance across all settings.
