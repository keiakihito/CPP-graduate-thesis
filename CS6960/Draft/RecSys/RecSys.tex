...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\columnwidth, keepaspectratio]{diagram/plots/va_scatter.pdf}
  \caption{Valence--arousal scatter of the 203-track iPalpiti corpus. AV predictions concentrate in a narrow mid-range band (approximately $0.25$--$0.55$ on both axes), with a positive correlation between dimensions. Dashed lines mark the 33rd and 67th percentile thresholds used to assign the extreme character labels (\textit{Energetic}, \textit{Calm}, \textit{Tense}). The clustering motivates percentile-based thresholding: fixed absolute cut-points would yield highly imbalanced or empty label classes in this domain. An additional inner boundary at the 40th--60th percentiles (not shown) defines the neutral \textit{Lyrical} class.}
