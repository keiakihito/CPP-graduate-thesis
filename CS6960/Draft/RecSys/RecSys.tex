...
Relevance labels for the proxy tasks are derived from metadata and a semi-automatic annotation pipeline. For the sanity proxy, composer labels are normalized to a canonical form using Unicode decomposition and a hand-curated alias table, ensuring consistent matching across recordings with variant spellings or abbreviations. This normalized field is stored as a stable evaluation key, separate from the raw metadata string.

For the primary musical proxy, character labels are constructed through a three-stage pipeline grounded in the arousal--valence (AV) framework \cite{eerola2011}, following standard deep learning-based MER approaches as surveyed in \cite{jiang2024}. Each track is processed using Music2Emo \cite{music2emo2025}, a pretrained music emotion recognition model that outputs valence and arousal scores on a 1--9 scale, linearly normalized to $[0,1]$ via $\hat{x} = (x-1)/8$. Because our corpus is entirely classical music, predicted AV values concentrate in a narrow mid-range region, making fixed absolute thresholds unreliable. As an empirical design choice, per-dimension thresholds are set at the 33rd and 67th percentiles of the dataset's own distribution, yielding four binary tags: \textit{Energetic} (arousal $\geq$ 67th percentile), \textit{Tense} (valence $\leq$ 33rd percentile), \textit{Calm} (arousal $\leq$ 33rd percentile), and \textit{Lyrical} (valence and arousal both within the 40th--60th percentile band). Because expert annotation was not feasible within the scope of this study, the model-generated tags are treated as pseudo-labels without professional validation.
