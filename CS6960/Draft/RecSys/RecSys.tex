...
This study is intentionally scoped to a single-domain classical music archive under cold-start conditions---a controlled setting that isolates the candidate generation component from user modeling, re-ranking, and catalog heterogeneity. This design allows a clean comparison of embedding capacity effects, but results should not be extrapolated to large-scale or multi-domain settings without further validation.

Relevance labels are derived from metadata rather than user interactions. Composer labels are deterministic and editorially grounded; character labels are pseudo-labels generated by a pretrained music emotion model and reflect model predictions rather than expert annotation. Because these labels are fixed and applied uniformly across all embedding models under evaluation, they affect absolute metric values but not the relative capacity comparisons, which are the primary focus of this study.

The evaluation isolates item-to-item similarity ranking and does not model downstream personalization or user-level re-ranking. Results characterize the candidate generation stage specifically; they do not speak to how these embeddings would perform in a full recommendation pipeline with user signals.
