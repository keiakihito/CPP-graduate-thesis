...
\begin{abstract}
This study investigates the effect of embedding-model capacity on content-based music similarity retrieval in a small-scale classical music archive. While larger models are often assumed to yield superior representations, their effectiveness in limited, single-domain settings remains unclear. We evaluate CNN-based and Transformer-based embedding models across multiple capacity tiers using two proxy tasks: composer-based retrieval and musical-character retrieval. Retrieval performance is assessed using ranking-based metrics (NDCG, Hit@K) and set-based metrics (Precision, Recall, and F1). Results show that increasing model capacity is not a reliable driver of retrieval performance in this setting. Notably, larger models do not consistently outperform medium-sized alternatives, despite substantial increases in parameter count. While higher-capacity models provide limited gains for structured tasks, scaling effects are weak or absent for abstract semantic retrieval. We further observe that evaluation outcomes are strongly influenced by the relationship between the number of relevant items and the evaluation cutoff. When many relevant items exist per query, set-based metrics such as Recall and F1 provide limited signal at small cutoff values, whereas ranking-based metrics such as NDCG and Hit@K remain more stable. These findings indicate that retrieval performance in small-scale archival settings is governed more by task structure and evaluation design than by model capacity alone.
\end{abstract}
...
These variants differ primarily in network depth, which determines model capacity within the CNN family. Although parameter counts provide a useful indication of model scale, they are not directly comparable across architectural families due to differences in architectural design and pretraining objectives. Therefore, capacity is defined within each family using standard scaling dimensions, while parameter counts are reported for reference.
...
The Sanity Proxy results show that increased capacity improves retrieval only under specific conditions when relevance is defined by a relatively structured and objective criterion. 
...
The experimental results yield three main observations. First, increasing model capacity does not reliably lead to performance improvement, even when model capacity increases substantially. While some gains are observed in the Sanity Proxy, these gains either saturate or remain limited to specific settings; on the Musical Proxy, scaling effects are weak or absent. Second, scaling behavior differs clearly by task. Structured composer retrieval benefits more from increased capacity, whereas abstract character-based retrieval does not show consistent improvements. Third, performance values cluster within a relatively narrow range across models, suggesting limited separability under the current retrieval setup. Taken together, these findings indicate that the effect of embedding-model capacity is strongly task-dependent. While increased capacity can improve retrieval when relevance is defined by structured and well-constrained attributes, it does not reliably improve performance for abstract semantic similarity in small-scale archival settings.
