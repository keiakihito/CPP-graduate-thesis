...
Table~\ref{tab:latency_results} shows that embedding extraction cost increases substantially with model capacity, particularly for the Transformer family. Notably, larger models incur significantly higher latency without consistently improving retrieval performance.

Importantly, the reported latency corresponds to offline embedding generation during catalog ingestion. Once embeddings are computed and indexed, online retrieval is performed via cosine similarity search and is independent of the embedding model. These results provide additional context for practical deployment, suggesting that lightweight or mid-sized models may offer a more favorable trade-off between computational cost and retrieval effectiveness in small-scale archival settings.

\subsection{Implications for Backend Design in Classical Music Archives}
For small-scale classical music archives, system design may benefit from prioritizing task definition, label quality, and dataset curation over model scaling. In practical archival systems such as iPalpiti, this suggests that efficient models may be sufficient for real-time retrieval and deployment-constrained environments. While this study focuses on item-to-item similarity retrieval to isolate embedding quality, this setup does not directly model user-level recommendation, where multiple items are aggregated into user profiles.
