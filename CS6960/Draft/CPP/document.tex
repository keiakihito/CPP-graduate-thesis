...
Across CNN and Transformer families spanning 4.8M to 330M parameters,
...
\begin{figure}[htbp]
  ...
  \caption{Simplified PANNs (CNN) architecture. Capacity scaling across Cnn6, Cnn10, and Cnn14 is achieved by increasing the number of convolutional blocks $N$, while the input preprocessing and output embedding stages remain structurally identical across tiers.}
  \label{fig:pann_arch}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{MERT pre-training framework \cite{li2023mert}. The Transformer encoder is trained via masked audio modeling supervised by acoustic (EnCodec VQ) and musical (CQT) teacher networks. Capacity scaling from MERT-95M to MERT-330M increases the depth and width of the encoder.}
  \label{fig:mert_arch}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{Cost--quality trade-off across all five embedding models on both proxy tasks. The $x$-axis shows CPU extraction latency (ms/track, log scale); the $y$-axis shows NDCG@5. Circle markers denote CNN-family models (PANNs); diamond markers denote Transformer-family models (MERT). The dashed line connects models in order of increasing capacity. On both tasks, NDCG@5 remains flat or declines as latency grows by up to ${\sim}25{\times}$, providing a unified visual summary of the central finding: capacity scaling does not produce consistent ranking gains relative to its extraction cost.}
