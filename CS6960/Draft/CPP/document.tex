...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.55\textwidth, keepaspectratio]{../RecSys/diagram/pipeline/RecSys-pipeline-v6.pdf}
  \caption{Overview of the evaluation pipeline. Blue boxes denote data artifacts; orange boxes denote key processes (embedding model, cosine similarity); the pink box denotes the proxy task evaluation stage. The embedding model is the only variable across experiments; all other components are held constant.}
  \label{fig:pipeline}
\end{figure}
...
Set-based metrics are further constrained by the relevance distribution. When the number of relevant items is large relative to the evaluation cutoff $K$, standard set-based metrics such as Recall@K can exhibit structural suppression, underestimating true retrieval effectiveness even when ranking quality is reasonable \cite{canamares2020}. Rank-aware metrics such as NDCG are more appropriate for retrieval-oriented settings because they directly evaluate the ordering of relevant items, unlike binary set-based measures which ignore position \cite{urbano2013}. This metric mismatch is observable in the character retrieval results: Hit@5 remains high ($\geq$0.749) across all models while Recall@5 is uniformly low ($\leq$0.039), reflecting structural suppression rather than poor retrieval.
...
The empirical results support the stated hypothesis. Increasing embedding-model capacity does not consistently improve candidate ranking quality: scaling is non-monotonic within both architectural families, and the largest model in each family fails to achieve the best ranking quality on either proxy task. The cost penalty for high-capacity models is substantial---a ${\sim}25{\times}$ latency increase from CNN-Small to Transformer-Large---without a corresponding improvement in NDCG@5 or Hit@5. The hypothesis holds across both structured and abstract retrieval tasks, though the mechanism differs: structured retrieval shows measurable but inconsistent capacity sensitivity, while abstract retrieval is largely insensitive to model size regardless of capacity tier.
...
The latency results reflect offline catalog ingestion cost, not online retrieval cost. Embeddings are computed once and stored in an index during ingestion; online retrieval is performed via cosine similarity search and is independent of the embedding model. Because the ingestion cost is paid upfront, the absolute values are less critical than they would be in a streaming setting---but for archival systems that re-index on catalog updates or lack GPU infrastructure, the ${\sim}25{\times}$ overhead difference between CNN-Small and Transformer-Large is operationally significant.

These findings also indicate that the relevance structure of the retrieval task can have a stronger effect than model size on metric behavior. When relevant items are broadly distributed---as in the character proxy---Recall@5 and F1@5 remain low regardless of model, while NDCG@5 and Hit@5 remain stable. Practitioners designing offline evaluation protocols for cold-start candidate generation may benefit from aligning metric choice with the expected relevance distribution of their catalog.
