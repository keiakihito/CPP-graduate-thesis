...
In addition to surveying model architectures and evaluation metrics, this review critically examines the implicit assumptions under which embedding-based retrieval evaluation is considered valid, particularly in small, domain-specific archives.
...
Some prior work applies ranking-based evaluation to music retrieval tasks, demonstrating that embeddings with strong classification performance do not always yield optimal ranking behavior \cite{tamm2024}. 
This distinction motivates the use of ranking metrics as the primary evaluation criterion in this thesis.
However, most ranking-based evaluations in the literature assume large candidate sets, stable relevance definitions, and statistically meaningful ranking distributions. The validity of such assumptions in small, curated classical archives has not been explicitly examined.
...
Moreover, prior work rarely makes explicit the boundary conditions under which ranking-based retrieval evaluation remains meaningful in constrained, small-scale settings. 
Factors such as normalization choices, similarity measures, proxy-label granularity, and candidate-set size are often treated as implementation details rather than as defining assumptions of the evaluation protocol.
In small-scale settings, these factors can substantially alter ranking outcomes, potentially leading to unstable or non-interpretable evaluation results. When the candidate set is limited and relevance signals are coarse, minor implementation choices may disproportionately influence measured performance.
...
Rather than assuming that standard large-scale evaluation protocols transfer unchanged to archival settings, this study explicitly investigates the stability and boundary conditions of embedding-based retrieval evaluation in small, content-driven archives. 
Within this boundary-aware framework, the work further examines whether increased model capacity translates into meaningful retrieval improvements when relevance is defined through musically motivated proxy tasks. 
By formalizing evaluation assumptions and empirically testing their limits, this thesis contributes a structured perspective on when and how embedding complexity matters in small, domain-specific music archives.
