...
In addition to surveying model architectures and evaluation metrics, This review supports the thesis focus on embedding-model capacity and its practical impact on retrieval performance in small-scale classical archives, while also discussing evaluation considerations relevant to such constrained settings..
The review is structured to support the thesis focus on backend-only comparison using ranking-based metrics, while placing collaborative filtering, user modeling, and system deployment in a background role.
...
Several studies apply CNN-based architectures to supervised music genre classification tasks \cite{dias_cnn, zhang2022}, though reported performance varies depending on dataset scale and model design.
...
Self-supervised learning approaches based on transformers have gained popularity due to their ability to leverage large unlabeled datasets. 
The Audio Spectrogram Transformer (AST) demonstrates state-of-the-art performance across multiple audio classification benchmarks \cite{gong2021ast}. 
These approaches represent a contrasting architectural family to CNN and CNN--RNN models, often trading increased model capacity for improved representational flexibility.
Transformer-based architectures typically benefit from large-scale pretraining and extensive data exposure. Due to their relatively weak inductive biases compared to convolutional models, their performance often scales with dataset size. This raises questions about whether such high-capacity architectures retain their advantage when applied to small, domain-constrained archives with limited data diversity.
...
For recommendation and retrieval systems, ranking-based metrics such as normalized discounted cumulative gain (NDCG), Precision@K, and Recall@K are more appropriate, as they evaluate whether relevant items are placed near the top of a retrieved list.
While ranking-based evaluation better reflects similarity retrieval behavior than classification metrics, its interpretation depends strongly on dataset characteristics. In small-scale archives with limited candidate sets and coarse proxy definitions, differences in model capacity may not translate into stable or interpretable ranking improvements. Minor variations in embedding representations can disproportionately influence measured performance when the evaluation pool is small.
Therefore, in constrained archival settings, ranking metrics must be interpreted in relation to dataset scale, proxy granularity, and model capacity.
As a result, the relationship between model capacity and ranking performance may become unstable in small-scale settings, making it unclear whether observed improvements reflect genuine representational gains or evaluation artifacts.
...
Prior studies consistently report that increasing architectural complexity and model capacity improves performance on large-scale classification benchmarks and heterogeneous music datasets.
...
Despite the central role of embedding capacity in modern audio representation learning, there is limited empirical work examining its practical effectiveness under such small-scale archival conditions.
...
Through a hypothesis-driven experimental design, the study examines whether heavier models provide measurable gains over lighter architectures and identifies conditions under which such gains emerge or disappear.
