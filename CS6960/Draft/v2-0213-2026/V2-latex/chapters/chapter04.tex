...
Segmentation is treated as a representation-level preprocessing step in V1, rather than as a primary factor for improving retrieval performance.
...
\textbf{Ground truth}: Relevance for each query is defined by the chosen proxy task.
For the sanity proxy, relevance is defined by composer identity.
For the primary musical proxy, relevance is defined by a coarse character label
(e.g., heavy vs.\ light).

Other metadata relationships, such as shared musical work or performer,
may be used as optional sanity checks to verify basic consistency,
but they are not treated as primary proxy tasks in V1.

The exact metadata fields and matching rules will be specified per proxy task
when the evaluation set is finalized.
This procedure yields one ranked list per query per model; these lists are the inputs to the evaluation step.


\section{Proxy Tasks (V1.2 Placeholder)}


\subsection{Sanity Proxy: Composer-Based Retrieval}
% Definition (to be finalized):
% Given a query track, retrieve top-K neighbors and measure composer-consistency as a sanity check.

\subsection{Primary Musical Proxy: Character Similarity (Heavy vs. Light)}
% Definition (to be finalized):
% A coarse musical-character label (e.g., heavy vs. light) is assigned to tracks.
% Evaluate whether nearest neighbors share the same character label more than chance.
...
\begin{itemize}
    ...
    \item Use of metadata-based proxy tasks for sanity checking, alongside a primary musical-character-based proxy for evaluation.
...
\begin{itemize}
    ...
    \item Proxy tasks: final metadata definitions and matching rules for the sanity proxy (composer-based) and the primary musical proxy (character similarity, e.g., heavy vs.\ light).
