...
Several studies evaluate CNN-based embeddings using supervised objectives, such as genre or tag classification, demonstrating strong performance with relatively modest model complexity \cite{dias_cnn}. These models are often attractive for deployment in resource-constrained environments due to their computational efficiency.
...
A parallel line of research focuses on modeling user behavior, preferences, and context to improve recommendation quality. Sequential recommendation models, emotion-aware systems, and geographically informed approaches incorporate user interaction data to personalize recommendations \cite{abbattista2024, schedl2021}.

At the system level, prior work also explores scalable architectures and deployment strategies for music recommendation platforms. These studies address challenges related to latency, scalability, and system integration. While such approaches are essential for large-scale commercial systems, they are secondary to the present thesis, which isolates backend embedding quality and evaluates it independently of user modeling and deployment concerns.

\section{Synthesis and Research Gap}

The reviewed literature demonstrates significant progress in learning audio embeddings for music-related tasks, spanning CNN-based, hybrid, and transformer-based architectures. However, several gaps remain. First, most evaluations focus on classification tasks or popular music datasets, limiting insight into ranking-based retrieval performance for classical music. Second, there is a lack of systematic comparison across embedding model families under a shared evaluation protocol tailored to archival settings.

This thesis addresses these gaps by focusing on backend audio embedding models and evaluating them on classical music similarity and retrieval tasks using ranking-based metrics. By isolating the embedding component and applying consistent evaluation criteria, the study aims to clarify how architectural design choices influence retrieval performance in classical music archives.
